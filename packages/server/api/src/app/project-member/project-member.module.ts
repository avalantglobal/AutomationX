import { ListProjectMemberQueryParams, ListProjectMemberRoleBody, PrincipalType, ProjectMember, ProjectMemberRole, SeekPage } from '@activepieces/shared'
...
const projectMemberController: FastifyPluginAsyncTypebox = async (app) => {
    app.get('/roles', ListProjectMemberRolesRequest, async () => {
        return [
            { name: 'Editor', value: ProjectMemberRole.EDITOR },
            { name: 'Viewer', value: ProjectMemberRole.VIEWER },
        ]
    })
...
const ListProjectMemberRolesRequest = {
    config: {
    ...
    schema: {
        tags: ['project-roles'],
        response: {
            [StatusCodes.OK]: Type.Array(ListProjectMemberRoleBody),
        },
    },
}
