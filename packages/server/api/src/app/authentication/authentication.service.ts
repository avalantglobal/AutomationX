...
export const authenticationService = (log: FastifyBaseLogger) => ({
  ...
  async signInWithPassword(
    ...
    const identity = await userIdentityService(log).verifyIdentityPassword(
      params
    );
  ...
  async federatedAuthn(
    ...
    const userIdentity = await userIdentityService(log).getIdentityByEmail(
      params.email
    );
  ...
  async switchPlatform(
    params: SwitchPlatformParams
  ): Promise<AuthenticationResponse> {
    const platforms =
      await platformService.listPlatformsForIdentityWithAtleastProject({
        identityId: params.identityId,
      });
    const platform = platforms.find(
      (platform) => platform.id === params.platformId
    );
  ...
  async switchProject(
    ...
    const projectPlatform = await platformService.getOneOrThrow(
      project.platformId
    );
    await assertUserCanSwitchToPlatform(
      params.currentPlatformId,
      projectPlatform
    );
...
async function assertUserCanSwitchToPlatform(
  ...
  const allowToSwitch =
    !platformUtils.isEnterpriseCustomerOnCloud(platform) || samePlatform;
  if (!allowToSwitch) {
    throw new ActivepiecesError({
      code: ErrorCode.AUTHENTICATION,
      params: {
...
async function createUserAndPlatform(
  ...
  await authenticationUtils.saveNewsLetterSubscriber(
    user,
    platform.id,
    userIdentity,
    log
...
async function getPersonalPlatformIdForFederatedAuthn(
  email: string,
  log: FastifyBaseLogger
): Promise<string | null> {
...
async function getPersonalPlatformIdForIdentity(
  identityId: string
): Promise<string | null> {
  ...
  if (edition === ApEdition.CLOUD) {
    ...
    const platform = platforms.find(
      (platform) => !platformUtils.isEnterpriseCustomerOnCloud(platform)
    );
